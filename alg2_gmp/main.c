...
int main (int argc, char **argv)
{
   ...
   * image: the name of the image file produced.*/
  ...
  /* All the processors work on the same image, and process 0 saves the image to .bmp */
  while (current_image <= end_image) {
    ...
    if (my_rank == 0) {
      ...
      //saveIterations(image1, iterations, width, height);
